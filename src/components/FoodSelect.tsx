    ...
    return (
        <div className="relative w-full" ref={wrapperRef}>
            <div
                ...
                tabIndex={0}
            >
                <span className={selectedFood ? "text-slate-900" : selectedId === '__custom__' ? 'text-blue-700 font-medium' : "text-slate-400"}>
            ...
            {isOpen && (
                <div className="absolute z-50 w-full mt-2 bg-white border border-slate-200 rounded-lg shadow-xl max-h-60 overflow-y-auto">
                    ...
                    <div className="py-1">
                        {/* Custom Food Option */}
                        <div
                            className={`px-4 py-3 cursor-pointer hover:bg-blue-50 text-sm border-b border-slate-100 flex items-center gap-2 ${selectedId === '__custom__' ? 'bg-blue-100' : ''}`}
                            onClick={() => {
                                onSelect('__custom__');
                                setIsOpen(false);
                                setSearch('');
                        ...
                        >
                            <svg className="w-4 h-4 text-blue-600 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                            <div className="flex flex-col">
                                <span className="font-semibold text-blue-700">Custom Food (Enter from Label)</span>
                                <span className="text-slate-500 text-xs">Manually enter your food's Guaranteed Analysis</span>
                            </div>
                        </div>
                        ...
                        {filteredFoods.length > 0 ? (
                            filteredFoods.map(food => (
                                ...
                                >
                                    ...
                                    <span className="text-slate-600 truncate">{food.name} <span className="text-slate-400 text-xs ml-1">({TYPE_LABELS[food.type] || food.type})</span></span>
                                </div>
                            ))
                        ) : (
